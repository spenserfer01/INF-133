...
carreras = ["Ingeniería de Sistemas", "Economía", "Medicina"]
...
class RESTRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed_path.path == "/estudiantes":
        ...
        elif parsed_path.path == "/carreras":
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps(carreras).encode("utf-8"))
        elif parsed_path.path.startswith("/estudiantes/"):
        ...
        elif parsed_path.path.startswith("/estudiantes/carrera/"):
            carrera = parsed_path.path.split("/")[-1]
            ...
            if estudiantes_carrera:
                ...
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"Error": f"No hay estudiantes en la carrera de {carrera}"}).encode("utf-8"))
        else:
            self.send_response(404)
            self.send_header("Content-type", "application/json")
            self.end_headers()
